...
export interface FakeChangeProviderOptions {
 ...
 */
export class FakeChangeProvider implements CompletionProvider {
  readonly calls: CompletionRequest[] = []
  private readonly gate: Deferred | null

  constructor(
    private readonly response: string | (() => string),
    private readonly options: FakeChangeProviderOptions = {},
  ) {
  ...
  async *stream(): AsyncIterable<string> {
    throw new Error('FakeChangeProvider.stream is not used')
  }
}
